...
<script>
	...
	function setAudioSrc(subject) {
		...
		$(".subjectAudioView").each(function(index, ele) {
			//遍历，怎么做
			var ele = $(ele);
			//ele.prop("src", '${pageContext.request.contextPath}/'+subject.audios[subject.indexList[index]]);//设置src
			ele.empty();
			var sourcEle=$("<source />");
			ele.append(sourcEle);
			sourcEle.prop("type","video/webm");
			sourcEle.prop("src",'${pageContext.request.contextPath}/'+subject.audios[subject.indexList[index]]);
			ele.prop("index", subject.indexList[index]);//设置src
		});
